...
class TimSort<T : Comparable<T>> {
    ...
    private fun insertionSort(array: Array<T>, start: Int, end: Int) {
        var i = start + 1
        while (i <= end) {
            val current = array[i]
            var j = i - 1
    ...
    private fun merge(array: Array<T>, start: Int, middle: Int, end: Int) {
        val leftSize = middle - start + 1
        val rightSize = end - middle

        var i = 0

        val leftArray = Array<Any?>(leftSize) { null }
        while (i < leftSize) {
        ...
        i = 0
        val rightArray = Array<Any?>(rightSize) {  }
        while (i < rightSize) {
            rightArray[i] = array[middle + i + 1] as T
            i++
        }
        ...
        while (i < leftSize && j < rightSize) {
            if (leftArray[i] as T <= rightArray[j] as T) {
                array[k] = leftArray[i] as T
                i++
            } else {
                array[k] = rightArray[j] as T
                j++
            }
            k++
        }
